...
class MonoDriver(Node):
    def __init__(self, node_name = "mono_py_driver"):
        ...
        self.image_topic_name = str(self.get_parameter('image_topic').value)
        ...
        self.subscribe_img_msg_ = self.create_subscription(Image, '/camera', self.run_py_node, 1)
    ...
    def run_py_node(self, img_msg):
        ...
        # img_msg = self.br.cv2_to_imgmsg(img_msg, encoding="passthrough")
        # cv2.imshow(img_msg)
...
def main(args = None):
    ...
    n = MonoDriver("mono_py_driver") #* Initialize the node
